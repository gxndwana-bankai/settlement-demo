use crate::chain_client::{ChainClient, ChainConfig, OrderProofJson, ProofData, Transaction};
...
impl ChainClient for SolanaClient {
    async fn submit_orders(
        ...
        if filtered_txs.is_empty() {
            println!("ℹ️  No orders found with source {}", self.config.chain.name());
            return Ok(());
        }
        ...
        for (i, tx) in filtered_txs.iter().enumerate() {
            ...
            instruction_data.extend_from_slice(&order.try_to_vec()?);
    ...
    async fn settle_orders(
        ...
        for order_proofs in proof_data.proofs_by_source_chain.values() {
            for order_proof in order_proofs {
                if order_proof.order.destination_chain_id == self.config.chain.chain_id() {
        ...
        let mut instruction_data = Vec::new();
        instruction_data.extend_from_slice(&discriminator);
        instruction_data.extend_from_slice(&sp1_public_inputs.try_to_vec()?);
        instruction_data.extend_from_slice(&groth16_proof.try_to_vec()?);
        instruction_data.extend_from_slice(&order_proofs.try_to_vec()?);
    ...
    async fn reset_orders(
        ...
        println!("📍 {} ({} orders)", self.config.chain.name(), order_hashes.len());
        println!("   Program ID: {}", self.program_id);
        ...
        let mut instruction_data = Vec::new();
        instruction_data.extend_from_slice(&discriminator);
        instruction_data.extend_from_slice(&order_hashes.try_to_vec()?);
